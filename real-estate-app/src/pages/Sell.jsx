...
const Sell = () => {
  ...
  const handleSubmit = async(e) => {
    ...
    try {
      const response = await axios.post("http://localhost:3000/sell", { propertyType, ...formData }, { withCredentials: true });
    ...
    >
      ...
      {step === 1 ? (
        <div className="bg-white p-10 rounded-xl shadow-2xl text-center max-w-lg">
          <h2 className="text-3xl font-bold text-gray-900 mb-6">What type of property are you selling?</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <motion.div 
              whileHover={{ scale: 1.05 }} 
              whileTap={{ scale: 0.95 }}
              className="p-6 border-2 border-gray-300 rounded-xl cursor-pointer hover:border-blue-800"
              onClick={() => { setPropertyType("house"); setStep(2); }}
            >
            ...
            <motion.div 
              ...
              whileTap={{ scale: 0.95 }}
              className="p-6 border-2 border-gray-300 rounded-xl cursor-pointer hover:border-blue-800"
              onClick={() => { setPropertyType("apartment"); setStep(2); }}
            >
        ...
        <div className="bg-white rounded-xl shadow-2xl w-full max-w-5xl grid grid-cols-1 md:grid-cols-2 overflow-hidden">
          {/* Form Section */}
          <div className="p-10">
            <h2 className="text-3xl font-bold text-gray-900 mb-6 text-center">List Your Property for Sale</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button
                type="submit"
                className="w-full bg-blue-600 text-white p-4 rounded-lg hover:bg-blue-700 transition font-bold text-lg"
              >
                List Property
              </button>
            </form>
              ...
              {formData.images.map((image, index) => (
                <div key={index} className="relative">
                  <img
                    ...
                    className="h-24 w-full object-cover rounded-lg shadow-md border"
                  />
                  <button
